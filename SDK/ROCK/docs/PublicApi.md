...
`ROCKAPI_QueryInterfaceV1` is the only discovery export. Register one Core owner, then bind each family's local permissions. There is no public all-features frame, global capability mask, or independent configuration export. All current majors are 1. Hands and WeaponParts are minor 1; [Weapon 1.1](modular/WeaponV1_1.md) adds inventory equip into either physical hand and retained switching, preserving the original Weapon 1.0 contract.
